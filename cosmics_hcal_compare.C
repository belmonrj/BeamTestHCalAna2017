...
void cosmics_hcal_compare(bool log_scale=false)
{
  TString det[2] = {"HCALOUT","HCALIN"};
  TString gain = "RAW";
  TLegend * legend = new TLegend( 0.5, 0.4, 0.88, 0.6, "", "NDC" );
  ...
  */
  ...
  char *file_tag[] = { "Outer HCal","Inner HCal"};
  //char *file_tag[] = { "Self Trigger","External Trigger"};
  TCanvas *c1 = new TCanvas(
                            det[0] + gain + TString(log_scale ? "_Log" : ""),
                            det[0] + gain + TString(log_scale ? "_Log" : ""),
                            750, 700);
          ...
          for(int ifile=0; ifile<2; ifile++)
            {
              ...
              TH1F * h = (TH1F*) fin->Get(Form("hEnergy_ieta%d_iphi%d", ieta, iphi)+ det[ifile]);
              //if(ifile==1 && ieta==0 && swap==true) { ieta=3; swap=false; }
              h->SetFillColor(0);
              h->Rebin(10);
              h->SetAxisRange(60,1200);
              h->SetLineWidth(2);
              ...
              text->Draw("Same");
              double min = 50;
              TF1 *f1 = new TF1(Form("fit1_ieta%d_iphi%d", ieta, iphi)+ det[ifile], "landau(0)",min,2000);
              h->Fit(f1,"QR0");
              //f1->Draw("Same");

              TF1 *f2 = new TF1(Form("fit2_ieta%d_iphi%d", ieta, iphi)+ det[ifile], "expo(0)",min,2000);
              h->Fit(f2,"QR0");
              //f2->Draw("Same");

              TF1 *f3 = new TF1(Form("fit3_ieta%d_iphi%d", ieta, iphi)+ det[ifile],"expo(3)+landau(0)",min,2000);
  ...
  c1->Print("figures/RB_cosmics_hcal_compare.png");
  c1->Print("figures/RB_cosmics_hcal_compare.pdf");
