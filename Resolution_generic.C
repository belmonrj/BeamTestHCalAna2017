...
void Resolution_generic()
{
 ...
 TH1F *res = new TH1F("res","res",280,0, en[nfiles-1]+5);
 res->SetMarkerStyle(20); res->SetMarkerSize(1.2);
 res->GetYaxis()->SetTitle("Resolution");
 res->GetXaxis()->SetTitle("Beam Energy");

 TH1F *linearity = new TH1F("linearity","linearity",280,0,en[nfiles-1]+5);
 ...
 for(int ifile=0; ifile<nfiles; ifile++)
 {
  ...
  T->Draw(Form("hcalin_e_new+hcalout_e>>%s",name.Data()),"good_h && abs(ahcal)<1.", "goff"); 
 ...
 // }
 if(en[ifile]==6) min_range = 1.8;
 if(en[ifile]==8) min_range = 2.4;
 ...
 res->SetMaximum(1.2);
 res->Draw("P");
 TF1 * f_calo_r = new TF1("f_calo_r", "sqrt(2*2+[0]*[0]+[1]*[1]/x)/100", 5, 65);
 f_calo_r->SetParLimits(0,0,30);
 f_calo_r->SetParLimits(1,0,200);
 res->Fit(f_calo_r, "RM", "", 5, 65);
 f_calo_r->SetRange(0.3,60);
 f_calo_r->Draw("Same");
